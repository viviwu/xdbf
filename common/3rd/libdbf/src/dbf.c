...
P_DBF *dbf_Open(const char *file, bool read_only )
{
	...
	if (file[0] == '-' && file[1] == '\0') {
	    ...
	    if(read_only==true){
            p_dbf->dbf_fh = open(file, O_RDONLY|O_BINARY);
	    }else{
            p_dbf->dbf_fh = open(file, O_RDWR|O_BINARY);
	    }
        if (p_dbf->dbf_fh == -1) {
...
int dbf_WriteRecord(P_DBF *p_dbf, char *record, int len) {
	...
	if(len != p_dbf->header->record_length-1) {
		fprintf(stderr, _("Length of record mismatches expected length (%d != %d)."), len, p_dbf->header->record_length-1);
		fprintf(stderr, "\n");
		return -1;
	}
