...
bool DbfDoc::open(const std::string& filename, bool read_only) {
  readOnly_ = read_only;
  auto mode = std::ios::binary | (read_only
                                    ? std::ios::in
  ...
  size_t expected_pos = sizeof(DB_HEADER) + fields_.size() * sizeof(DB_FIELD) +
                        1;
  ...
  filename_ = filename;
  if (read_only) isModified_ = false;
...
bool DbfDoc::modifyRecord(uint32_t index,
...
bool DbfDoc::addRecord(const std::vector<std::string>& data) {
  if (!validateData(data)) {
    std::cerr << "data.size()=" << data.size() << " != fields_.size()=" <<
        fields_.size()
        << std::endl;
    return false;
  }
  ...
  for (size_t i = 0; i < fields_.size(); ++i) {
    ...
    std::string value = data[i].substr(0, fields_[i].field_length);
    value.resize(fields_[i].field_length, ' ');
    std::memcpy(new_record.data() + offset, value.data(),
...
bool DbfDoc::create(const std::string& filename,
  ...
  header_.header_size = sizeof(DB_HEADER) + fields_.size() * sizeof(DB_FIELD) +
                        1;
...
void DbfDoc::debugPrint() const {
  ...
  std::cout << "  Expected Length: " << (calculateRecordLength() + 1) <<
      std::endl;
  std::cout << "  Num Records: " << header_.num_records << std::endl;
  std::cout << "  Fields: " << fields_.size() << std::endl;
  for (size_t i = 0; i < fields_.size(); ++i) {
    std::cout << "    " << fields_[i].field_name << ": " << (int)fields_[i].
        field_length
        << std::endl;
  }
}