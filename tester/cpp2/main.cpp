...
int read_dbf(int argc, char *argv[])
{
  cout << __func__ << endl;
  char date_str[9] = {0};              // 20230418
  char file_name[128] = {0};       // "OrderAlgo_20230510.dbf";

  if (argc>1 && sizeof(argv[1])<128){
    strncpy_s(file_name, 127, argv[1], sizeof(argv[1]));
  }else{
  ...
  for (int j = 0; j < row_cnt; j++) {
    record[rec_len] = 0;
    dbf.ReadDbfRecordAtRow(record, 1024);
