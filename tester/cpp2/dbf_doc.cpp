...
header_(0), fields_(0), columns_(0), cur_record_(0)
{
  if (file[0] == '-' && file[1] == '\0') {
    dbf_fs_ = fstream(stdin);
  } else{
    if(read_only){
      dbf_fs_.open(file, ios::in|ios::binary);  // r
    } else{
      dbf_fs_.open(file, ios::in | ios::out|ios::binary); // r+
    }
    if(!dbf_fs_.is_open()){
...
{
  if (file[0] == '-' && file[1] == '\0') {
    dbf_fs_= fstream(stdout);
  } else {  // truncate existing file or create for writing
    dbf_fs_.open(file, ios::out|ios_base::trunc|ios::binary); // O_CREAT
    return /* NULL */;
  }
...
int DbfDoc::ReadDbfHeaderInfo()
{
  ...
  // dbf_fs_.seekg(0, ios::beg);
  dbf_fs_.read((char*)header, sizeof(DB_HEADER));
...
int DbfDoc::WriteDbfHeaderInfo(DB_HEADER *header)
{
   ...
   * been written.
   */
  dbf_fs_.seekp(0, SEEK_SET); // seekp for write SEEK_SET

  dbf_fs_.write((char *)newheader, sizeof(DB_HEADER));
...
int DbfDoc::ReadDbfFieldInfo()
{
  ...
  dbf_fs_.seekg(sizeof(DB_HEADER), SEEK_SET);
  //lseek(dbf_fh_, sizeof(DB_HEADER), SEEK_SET);
  dbf_fs_.read((char *)fields, columns * sizeof(DB_FIELD));
...
{
  dbf_fs_.seekp(sizeof(DB_HEADER), SEEK_SET); 
  dbf_fs_.write((char *)fields, numfields * sizeof(DB_FIELD));
...
int  DbfDoc::ReadDbfRecordAtRow(char *record, int row) {
  ...
  cur_record_= row;
  dbf_fs_.seekg(header_->header_length + cur_record_ * (header_->record_length), SEEK_SET);

  dbf_fs_.read(record, header_->record_length);
  cur_record_++;
...
int  DbfDoc::WriteDbfRecord(char *record, int len) {
  ...
  dbf_fs_.seekp(0, ios::end);
//  lseek(dbf_fh_, 0, SEEK_END);
  dbf_fs_.write(UN_DELETE_FLAG, 1);
//  if (write( dbf_fh_, UN_DELETE_FLAG, 1) == -1 ) {
//    return -1;
//  }
  dbf_fs_.write(record, header_->record_length-1);
...
int  DbfDoc::DbfWriteRecordWithFlag(char *record, int len) {
  ...
  dbf_fs_.seekp(0, ios::end);
//  lseek(dbf_fh_, 0, SEEK_END);
  dbf_fs_.write(record, header_->record_length);
